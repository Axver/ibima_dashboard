...
class Pembelian_produk extends CI_Controller
{
	...
	{
		$tujuh=$this->db->query('SELECT count(id_pembelian) as one FROM pembelian_produk WHERE DATE(created_at) = DATE(NOW() - INTERVAL 1 DAY )')->result();
		$enam=$this->db->query('SELECT count(id_pembelian) as one FROM pembelian_produk WHERE DATE(created_at) = DATE(NOW() - INTERVAL 2 DAY )')->result();
		$lima=$this->db->query('SELECT count(id_pembelian) as one FROM pembelian_produk WHERE DATE(created_at) = DATE(NOW() - INTERVAL 3 DAY )')->result();
		$empat=$this->db->query('SELECT count(id_pembelian) as one FROM pembelian_produk WHERE DATE(created_at) = DATE(NOW() - INTERVAL 4 DAY )')->result();
		$tiga=$this->db->query('SELECT count(id_pembelian) as one FROM pembelian_produk WHERE DATE(created_at) = DATE(NOW() - INTERVAL 5 DAY )')->result();
		$dua=$this->db->query('SELECT count(id_pembelian) as one FROM pembelian_produk WHERE DATE(created_at) = DATE(NOW() - INTERVAL 6 DAY )')->result();
		$satu=$this->db->query('SELECT count(id_pembelian) as one FROM pembelian_produk WHERE DATE(created_at) = DATE(NOW() - INTERVAL 7 DAY )')->result();
